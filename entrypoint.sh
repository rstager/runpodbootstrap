...
# 1. Create coder user with home on /workspace (persists across pod restarts)
mkdir -p /workspace/home
if ! id coder &>/dev/null; then
    useradd -m -d /workspace/home/coder -s /bin/bash coder
fi

# 2. Grant passwordless sudo (idempotent — overwrites same file each run)
mkdir -p /etc/sudoers.d
echo 'coder ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/coder
chmod 0440 /etc/sudoers.d/coder

# 3. SSH — ensure coder can connect with the same key as root
